...
<body>
  ...
  <div id="hiddenNavigationBar">
    ...
    <div id="mainContainer">
        <div id="brandImage">
            <?php
            $bImage = "SELECT * FROM `brands` WHERE `brand_name`='$brandName'";
            $run = mysqli_query($con, $bImage);
            if ($run) {
                    $rowB = mysqli_fetch_assoc($run);
                    echo "<img src='Brands/". $rowB['back_image']."' alt=' $rowB[brand_name]'>"; 
                }
            ?>
        </div>
        <div id="brandTitle">
    ...
    <?php
    $query = "SELECT * FROM `products` WHERE `p_brand`='$brandName'";

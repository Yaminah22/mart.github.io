    ...
    <div id="mainContainer">
        ...
        <div class="addDiv">
            <form method="POST" enctype="multipart/form-data">
                <h2 class="title">Update Product Quantity</h2>
                ...
                <button type="submit" name="updateProduct" class="btn">Update Quantity</button>
            </form>
        </div>
    </div>
    <?php
    ...
    if (isset($_POST['updateProduct'])) {
        ...
        if ($result) {
            if (mysqli_num_rows($result) == 1) {
                ...
                $q = $sum['p_quantity'];
                $q = $q + $_POST['quantity'];
                $update = "UPDATE `products` SET `p_quantity`='$q' WHERE `product_name`='$_POST[name]'";
                $run = mysqli_query($con, $update);
                if ($run) {
                    echo "<script>alert('Quantity Updated Successfully!')
                    window.location.href='updateQuantity.php';
                    </script>";
                } else {
                    echo "<script>alert('Quantity Update Failed!')
                    window.location.href='updateQuantity.php';
                    </script>";
                }
            } else {
